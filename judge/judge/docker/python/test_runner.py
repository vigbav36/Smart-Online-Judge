...
def run_test_case(testcase):
    ...
    try:
        output, error, time_taken = run_user_code(
            "user_code.py",
            testcase.get('input'),
            testcase.get('timeLimit'),
            testcase.get('memoryLimit')
        )
        ...
        if error:
            ...
            if output.strip() == testcase.get('expectedOutput').strip():
                result["status"] = "PASSED"
            else:
                result["status"] = "WRONG_ANSWER"
                result["message"] = f"Expected: '{testcase.get('expectedOutput')}', Got: '{output}'"

        result["timeTaken"] = time_taken
    except Exception as e:
